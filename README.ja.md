...
| HWID             | Fixed Length    | 5byte    | ビーコンデバイスの識別ID。<br>HWIDはデバイスごとに指定してください。自身のBotアカウントに対して発行されたHWID以外は使わないでください。<br>HWIDは、[LINE OFFICIAL ACCOUNT MANAGERページ]((https://manager.line.biz/beacon/register))から発行できます。 |
